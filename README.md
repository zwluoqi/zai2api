...
> **Token 重试次数 / 模型降级链**：在管理面板「系统设置」中配置并持久化到 `config.json`（不通过环境变量）。模型降级链在请求模型返回容量满 `MODEL_CONCURRENCY_LIMIT` 或账号无权限时，依次降级到备用模型（默认 `GLM-5-Turbo`）；Token 重试次数默认 0（不额外重试）。
